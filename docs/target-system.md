...
| **L2 True talent** | K% BB% HR/PA BABIP ISO — tuned Marcel live; Bayesian arm DRAWS with it on K%, BB% and HR/PA (BAS-69, BAS-73); structural track: covariates as a regressor in the likelihood MEASURED and worse (BAS-83, errors-in-variables on the thin current window; the block is the negative control now), joint multi-component MEASURED (BAS-84: correlations real in 3 of 4 seasons, a reliable third of a percent on K%/BB% over the single-component arm, nothing on HR/PA, loses to contact quality everywhere; not served), measurement model IN FLIGHT (BAS-85, grid running); park factors IN FLIGHT (BAS-86, `docs/park-factors.md`: the offset the Bayes arms always expected has never existed) | K% BB% HR/BF BABIP-against, WHIP rate — **gated**, all five clear | framing runs, fielder runs — **in progress** (framing); no Marcel equivalent exists |
