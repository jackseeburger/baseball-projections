...
|---|---|---|---|---|---|
| **A. Player rates** | K%, BB%, HR/PA, ISO, BABIP projections. The site's live number is the **rest-of-season** one | Preseason: Marcel (K% MAE .0261 on 2026). Rest-of-season: `marcel_tuned_preseason` — the same tuned Marcel with the current season withheld | Preseason: Depth Charts .0234 · ours .0271 (last). **Rest-of-season: tuned Marcel fed the partial season, K% MAE .0269/.0293/.0341 at the May 1 / Jul 1 / Aug 1 cutoffs — beats our preseason Bayesian on 10 of 12 component-cutoff cells** ([ros-projections.md](ros-projections.md)). The tuning itself: per-component ballast, recency and age curve fitted walk-forward on 2020–24, frozen in `src/eval/marcel_params.json`, then scored on 2025, 2026 and the three 2026 cutoffs — **17/25 cells, pooled −1.10% ± 0.30 of stock Marcel's MAE**, with no component worse than stock: BABIP (−3.0%), K% (−1.4%), ISO (−1.1%), HR/PA (−0.5%); BB% keeps Tango's constants ([backtest-baselines.md](backtest-baselines.md#tuning-marcel--fitted-constants-beat-tangos-defaults)) | **The Bayesian arm finally got a fair fight (Sept 3)**: given a dated cutoff on PA rows and prior seasons rebuilt from Statcast, the same K% model refit at each cutoff scores .0278/.0302/.0341 against tuned Marcel's .0269/.0293/.0341 — paired gaps of +.0009/+.0009/+.0000 (t 1.6/1.6/0.0, n 315/231/126), so it no longer loses significantly at any cutoff, and it beats its own preseason self by 6–11%, which means the deficit the published table had been charging it with was the withheld season rather than the model ([backtest-baselines.md](backtest-baselines.md#the-fair-fight--the-bayesian-arm-refit-at-the-cutoff-bas-59)); level is still not a win, so Bayesian components **lose to Marcel** → still not wired. The **opposing-pitcher term added alongside it is the cautionary half**: PSIS-LOO prefers it by 280 nats at 11.3 dSE, and the one rest-of-season cutoff affordable with it on scores 5.9% *worse* (K% MAE .03615 against .03415), because the projection is made at a neutral pitcher and discards exactly what the term learned — LOO measured held-out plate appearances, the gate measures the rest of a season, and only the second one counts. **Tuned Marcel-with-partial is wired and live (Sept 2)**: it is what the player and leaderboard pages serve, with the preseason Bayesian numbers beside it as a labelled comparison. The **age curve was refit under a constraint (Sept 3)** — peak in 25–31, slopes of opposite signs, so it cannot double as a level correction — and the HR/PA and ISO gains that used to vanish out of sample now survive, while the live board's mean K% comes back from 0.7 pts under league to 0.2 under; the projected-league-rate options built alongside it lose their own inner validation and only ISO takes one. **The pitcher half of the station went in the same day (Sept 3)**: the same estimator pointed at the other id on the plate appearance, scored on five walk-forward cells against league average, the previous season and season to date, and all five components — K%, BB%, HR/BF, BABIP against and the walks-plus-hit-batsmen rate station E reads — clear the gate against all three, K% by 3.5% of stock's MAE after tuning and BABIP against by nothing at all ([backtest-baselines.md](backtest-baselines.md#the-pitcher-side-of-station-a--sept-3-2026)); `src/sim/starters.rate_table` now *calls* that provider rather than re-implementing it, so station A and station E cannot drift, and the station E scoreboard below is unmoved (`pythag_C_sp_bpa_ip` still .24388) because station E keeps stock's constants until it is re-scored. **Statcast is finally inside a model, and it clears the gate (Sept 3)**: six exit-velocity and launch-angle aggregates per player — mean EV, EV90, barrel, hard-hit, sweet-spot, mean launch angle, computed strictly before each cutoff from a committed monthly artifact — added as covariates on top of `marcel_tuned` beat it on all five hitter components and four of five pitcher ones, walk-forward on 2022–2026 at the three cutoffs: **ISO −4.4% of MAE (t −4.07, n 4,663), HR/PA −4.8% (t −4.48), K% −2.1% (t −4.68), BABIP −1.7% (t −2.55), BB% −1.6% (t −3.23); pitcher HR/BF −4.8% (t −5.86), BABIP against −1.7% (t −3.00)**, t clustered by player, every constant fitted on seasons before the one scored and a permuted-covariate control landing exactly on the recalibration arm. **The pitcher walk rates are the cautionary half**: they show t −5.6 against the baseline and *nothing* against a recalibration control, so that gain is a level correction and not Statcast at all. Splitting by exposure answers the question the ticket asked — BABIP and the hitter walk and strikeout rates are a **variance reduction** (the gain lives in the low-exposure half and is gone by August), while **ISO and pitcher HR/BF are genuine information** (as large or larger for the players with the most batted balls, and surviving the August cutoff). **Stage 2 is a negative result**: an HSGP surface over (EV, LA) with the batted ball's wOBA as the response — the tutorial's own technique, refitted at every fold — is *significantly worse* than the six hand-chosen aggregates on every component of both sides (hitter ISO +.001767, t +6.75) and adds nothing beside them, because averaging the league's value function over a player's batted balls returns roughly what those balls were worth, which is roughly what the outcome rate the baseline already reads says ([contact-quality.md](contact-quality.md)). **Wired (BAS-72)**: `LIVE_ENGINE` in `src/projections/ros.py` reads `contact_additive` on all five hitter components — the gate re-ran clean on the committed harness with the same numbers as above, and `contact_additive` clears it too on its own paired diffs (docs/contact-quality.md §8). §8's judgement call is the one shipped: the baseline's coefficient is pinned at exactly 1 and contact quality is added as a pure correction on top of `marcel_tuned`, left otherwise untouched, rather than the free fit that also rescales the baseline — giving up roughly a third of the free fit's gain in exchange for not smuggling a claim about Marcel's own ballasts in under a Statcast change. Features are read as of the last contact-quality month boundary on or before the as-of date rather than a partial-month top-up (`ros.contact_cutoff`), so a build made mid-month lags the as-of date by up to a month rather than reading a rounded-forward, leaked bucket — the served document's `contact_features_through` says exactly what date the features cover. Pitcher components stay on `marcel_pitcher_tuned`: the walk rates' gain above is pure recalibration and pitcher K% does not clear at all | `src/projections/ros.py` (`LIVE_ENGINE`), `pitcher_ros.py`, `scripts/build_ros_projections.py`, `public/data/projections/`; `src/eval` providers (`pitchers.py`, `contact.py`, `hsgp_contact.py`); `src/data/contact_quality.py`, `scripts/build_contact_quality.py`, `scripts/run_contact_backtest.py`; `src/eval/marcel_params.json`, `marcel_pitcher_params.json`; `data/features/contact_quality_monthly.parquet`, `data/projections/*.parquet` |
...
| **D. Team strength** | Talent win% | Every team .500 (coin flip) | Regressed Pythagenpat, 60-game ballast | **Wired, live** — and it is a *point estimate on purpose* as of Sept 3. The season Monte Carlo can now draw a fresh strength vector per simulated season (`strength.StrengthDistribution`, zero width bit-identical to the point path), and the width the 60-game ballast's own normal-normal posterior implies — .0505 of talent win% in April falling to .0313 in the last fortnight — was scored through the full station G backtest: **playoff Brier +.00015 (t +0.11), projected wins +.0104 of MAE (t +4.18) against the served chain on 7,470 club-projections**. It fails the gate, and the reason is that the served board is not over-confident: the best-fitting linear shrinkage of its playoff probabilities is 0.968, and of its pennant probabilities **1.031** — i.e. *sharpening*. [parameter-uncertainty.md](parameter-uncertainty.md) | `src/sim/strength.py` |
| **E. Per-game P(win)** | Home win prob for one game | log5 + HFA on team strength (Brier .2462) | **+ starting pitcher: .2448** (wired, live) · + posted lineup + bullpen quality: **.24454** · + station C's run environment: **.24428** · **+ the pen that is actually available, weighted by pitch counts: .24400** · **+ the starter's own expected innings as the workload split: .24388** · Kalshi **.2416** / Polymarket **.2417** (756 games, measured — [market-benchmark-2026.md](market-benchmark-2026.md)). On a Sept 3 re-pull of both archives (**737** games priced by both venues, the exchanges' own listings having rolled forward): pythag_60 .24654 → chain .24396 → **+ that same expected start length as a *level*: .24364** · Kalshi **.24201** / Polymarket **.24212** | **Wired, live** (Sept 2: the starter term; **Sept 3: the whole chain, and then the expected-innings level on top of it**) — one function (`src/sim/game_model.py`) prices a game for both the harness and the nightly, so every remaining game with both probables posted is served as `pythag_C_sp_bpa_ip_lvl` — station C's run environment, the starter over his own expected innings, the availability-weighted pen over the rest, a runs-allowed level for how deep that start is expected to go, and the posted card when a club has published one. Every term above the starter clears the gate on the common game set, and each is < .0006 and inside 1 SE there, so one more season is what would size them. **Availability itself is worth nothing** (a pitch-count weight fires on 1,485 of 1,512 club-games where the old binary rest rule fired on 12, and scores the same); what pays is pricing the relief innings off component rates against the league. The start-length term is the only one whose *sign* is significant anywhere (t = −2.9 on 2025). **Park and team defence were added to the same ladder on Sept 3 and did not clear it** — .24382 and .24395 against the served .24388, with the sign flipping between seasons on the first and negative on both on the second — so neither went in and the harness scores them as `pythag_C_sp_bpa_ip_pk`, `_def` and `_pk_def`, still sitting on the chain as it was that day rather than on the one below. **A learned challenger has now been tried and does not clear the gate (Sept 3)**: a gradient-boosted model over 23,193 games of 2015-2026, given the same pre-game inputs and no functional form at all, scores .24461 on the 756 against the chain's .24388 (paired +.00073, se .00123) — better on all of 2026 (.24536 vs .24592) and on 2025 (.24339 vs .24360), worse where the gate is, inside one standard error everywhere, and 88-94% correlated with the chain. It re-derives the chain's own terms in the chain's own order; the two places it disagrees are the starter's expected innings as a *level* and a pen delta at nearly twice the chain's weight. Available behind `backtest_game_odds.py --learned`; the chain runs ([market-benchmark-2026.md](market-benchmark-2026.md#station-e--a-model-that-chooses-its-own-form-sept-3-2026)). **The first of those two disagreements has now been spent, and it clears (Sept 3, issue #66)**: the same expected innings the chain used only to divide the game between two staffs are now also a runs-allowed level — a club is charged 0.9 runs per nine for each inning of the flat 5.5 its starter is not expected to cover — worth **−.00032 (se .00036, t −0.89, n 737)** on the market's own games, −.00023 on all 1,787 of 2026 and −.00017 on all 2,105 of 2025, the same sign on all three and the largest single term added here since the starter himself. The constant is a one-parameter maximum-likelihood fit on the **eleven** completed seasons before the scored one (0.89 ± 0.30, shipped at 0.9), and the term is better in 9 of 11 walk-forward folds (pooled −.00010, t −1.73, n 21,088). Two alternative encodings were tested: the same level charged only over the starter's own innings is indistinguishable on the prior pool and rejected for being negative at one point of its grid, and an **interaction with starter quality — the obvious alternative — is worth nothing** (−.00002 on the prior pool, −.00002 held out), which is what says the information is a level and not a weight on the FIP the chain already has. The market's residual edge is now **.00163** on that set ([market-benchmark-2026.md](market-benchmark-2026.md#station-e--the-lead-was-real-expected-innings-as-a-level-sept-3-2026)) | `strength.home_win_prob`, `src/sim/game_model.py`, `starters.py`, `lineups.py`, `bullpen.py`, `reliever_usage.py`, `run_environment.py`; `scripts/backtest_game_odds.py`, `sweep_starter_ip_level.py` |
...
biweekly as-of dates), [contact-quality.md](contact-quality.md) (A, what Statcast's
batted-ball data is worth on top of them — and what the GP version is not),
[playoff-odds-validation.md](playoff-odds-validation.md) (G, the
...
[parameter-uncertainty.md](parameter-uncertainty.md) (D and G — a distribution
over team strength instead of one number per club, scored on the same 249
dates and failing the gate).
...
| 2 | **Per-game with starters, lineups, bullpen** | Public projection systems don't publish game odds; the market does. Team strength gets Brier .2462; both exchanges' closes get .2416–.2417 on the same 756 games. A regressed-FIP starter term takes **0.0014 of that 0.0046** (.2448); station C's run environment, the available bullpen and the starter's expected innings take another **0.0009** between them (.24388), and reading those same expected innings a second time — as a level on runs allowed rather than only as the split — takes **0.0003** more (Sept 3, issue #66). On the Sept 3 re-pull of both archives that is .24654 → .24364 against Kalshi's .24201, leaving **0.0016** for weather, rest and a better pitcher model — measured, see market-benchmark-2026.md. Bullpen *availability* has now been tested twice and is worth nothing either time, and **park and team defence have now been tested once each and are worth nothing either**: the park factors predict 2026's park run environments at r = 0.44 and still move a game's probability by 0.0005, because they are symmetric. | Walk-forward Brier vs. market closing lines | E |
| 3 | **Calibrated uncertainty** | Everyone publishes a point estimate. Contract valuation (Phase 6) needs a distribution; a 10th/90th band that actually covers 80% is a product no one sells. **At station G this has now been tried and there was nothing to collect** (Sept 3): the season sim was given a distribution over team strength rather than one number per club, and the playoff board did not move (+.00015 of Brier, t +0.11) because it was already calibrated — reliability .00055, best-fitting shrinkage 0.968 on playoffs and 1.031 on pennants. The pocket is still open at station A, where a *player-level* band has never been coverage-tested; at G it is closed. [parameter-uncertainty.md](parameter-uncertainty.md) | Coverage tests (roadmap 5.7) | A, ~~G~~ |
| 4 | **Statcast-informed rates** | Marcel/Steamer/ZiPS use outcomes; batted-ball and swing data lead outcomes. **Measured Sept 3 and the pocket is real, at 1.6–4.8% of component MAE** — bigger than tuning Marcel's constants bought, smaller than giving Marcel the current season. Two things the measurement changed about the thesis: most of it is a *denoiser* on small samples rather than new information (only hitter ISO and pitcher HR/BF survive at large exposure and late cutoffs), and the *nonparametric* version is worse than six hand-chosen aggregates, because a single expected-value-on-contact number collapses back onto the outcome rate the model already has. | Same harness, add features — done: [contact-quality.md](contact-quality.md) | A |
