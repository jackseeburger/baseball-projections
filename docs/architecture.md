...
| **M. Market** | Archive of prices from **exchanges** (Kalshi, Polymarket: bid/ask, last, volume, settlement — public APIs, no key) and **sportsbooks** (closing lines via The Odds API / SportsbookReview); de-vig; score E/A/G against both; CLV + fill-aware simulated ROI | The market price itself | — | **Exchanges wired** — `market-snapshot.yml` archives ~600 Kalshi + ~3,100 Polymarket MLB markets 3×/day to `data/market/snapshots/` (immutable, gzipped), game markets mapped to `game_pk` (>99% of moneylines). Sportsbook lines (needs `ODDS_API_KEY`) and the 2026 pre-game-close backfill from Kalshi candlesticks / Polymarket price history are the open halves of issue #6 | `src/market/` |
