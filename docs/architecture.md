...
| **G. Odds** | P(playoffs/div/bye/pennant/WS), win bands | — | Live, 20k sims | **Wired, live** — since Sept 3 the board is served off the full chain (E on the announced games, C's blend on the rest and in the bracket), with `--legacy-chain` for the previous answer and `--coin-flip` for the no-model control the validation doc scores against | `src/sim/odds.py`, `scripts/run_playoff_odds.py` |
