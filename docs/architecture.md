...
| **B. Playing time** | PA per player, rest of season | Season-to-date PA share (MAE 25.8 PA over one month, 43.4 over two; equal-share floor 28.5) | Horizon blend of the 30-day and season shares, IL zeroed, one-lineup-slot cap — `w(h)` fitted on 2025 only: **MAE 21.9**, top-9 capture **.768** at one month (beats season-to-date by 3.9 PA, 3.7 SE, and the old 30-day model by 0.15); **at two months it is 45.4 and still loses** to season-to-date's 43.4, and no blend weight closes that — the two-month gap is the IL zeroing, not the window ([playing-time.md](playing-time.md) §4). 616/595 hitters, walk-forward at 2026-07-01 and 2026-08-01 | Built; **beats every baseline at the ~26-game horizon it serves**, loses to the naive season share at two; not yet wired | `src/projections/playing_time.py`; `data/parquet/playing_time_ros.parquet` |
...
| **M. Market** | Daily archive of prices from **exchanges** (Kalshi, Polymarket: bid/ask, last, volume, open interest — public, no key) and **sportsbooks** (~30 books incl. Pinnacle via The Odds API, de-vigged); 2026 pre-game closes reconstructed for both exchanges, plus the **hourly pre-game price path** (876 Kalshi markets, 20,989 candles); score E/A/G against all three | The market price itself | Kalshi ≈ Polymarket within ~1 pt on the same games. Fill-aware P&L now runs **on both sides of the book**. Taking: **every station-E model loses money at every edge threshold on both venues** — the full stack returns **−11.6% ROI** on 405 Kalshi bets at ≥2 pts (CI −22.3% to −1.3%), against −6.2% for a random-edge control; 4.4 pts of that is the fee, 4.0 the spread, 3.2 the model. Making: resting a limit order instead recovers **7.8 of those 8.4 points** and still returns **−3.1%** (CI −21.7% to +15.2%, 142 fills of 377 posted at a 5-pt margin), no better than its own shuffled-edge control and exactly where the taker exam's frictionless row already put it — **execution was the whole difference between losing 11 points and losing 3, and there is no edge underneath either** ([money-exam-2026.md](money-exam-2026.md)) | **Wired** — `market-snapshot.yml` 3×/day, closes in `market_closes_2026.parquet`, candles in `data/market/kalshi_candles_2026.parquet`, both exams in `src/market/pnl.py` (`scripts/money_exam.py [--maker]`); true CLV — entries scored against a later close — waits on the 3×/day snapshot archive | `src/market/`, `pnl.py`, `backfill.py` |
