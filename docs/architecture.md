...
page without either builder being edited.

## 2. Station status

| Station | What it is | Baseline to beat | Current best (Sept 1) | Status | Swap point |
|---|---|---|---|---|---|
| **A. Player rates** | K%, BB%, HR/PA, ISO, BABIP projections. The site's live number is the **rest-of-season** one | Preseason: Marcel (K% MAE .0261 on 2026). Rest-of-season: `marcel_tuned_preseason` — the same tuned Marcel with the current season withheld | Preseason: Depth Charts .0234 · ours .0271 (last). **Rest-of-season: tuned Marcel fed the partial season, K% MAE .0267/.0292/.0346 at the May 1 / Jul 1 / Aug 1 cutoffs — beats our preseason Bayesian on 10 of 12 component-cutoff cells** ([ros-projections.md](ros-projections.md)). The tuning itself: per-component ballast, recency and age curve fitted walk-forward on 2020–24, frozen in `src/eval/marcel_params.json`, then scored on 2025, 2026 and the three 2026 cutoffs — **15/25 cells, pooled −1.10% ± 0.36 of stock Marcel's MAE**, all of it BABIP (−3.3%) and K% (−2.4%); BB% keeps Tango's constants ([backtest-baselines.md](backtest-baselines.md#tuning-marcel--fitted-constants-beat-tangos-defaults)) | Bayesian components **lose to Marcel** → still not wired. **Tuned Marcel-with-partial is wired and live (Sept 2)**: it is what the player and leaderboard pages serve, with the preseason Bayesian numbers beside it as a labelled comparison. Caveat on the record: the fitted **age curve is the weak part** — its in-sample gains on HR/PA and ISO vanish out of sample, because the search uses it as a level correction for regressing to one season's league rate with three seasons of history; the live board's mean K% now sits 0.7 pts under league where stock sat on it | `src/projections/ros.py` (`LIVE_ENGINE`), `scripts/build_ros_projections.py`, `public/data/projections/`; `src/eval` providers; `src/eval/marcel_params.json`; `data/projections/*.parquet` |
...
| **E. Per-game P(win)** | Home win prob for one game | log5 + HFA on team strength (Brier .2462) | **+ starting pitcher: .2448** (wired, live) · + posted lineup + bullpen quality: **.24454** · + station C's run environment: **.24428** · **+ the pen that is actually available, weighted by pitch counts: .24400** · **+ the starter's own expected innings as the workload split: .24388** · Kalshi **.2416** / Polymarket **.2417** (756 games, measured — [market-benchmark-2026.md](market-benchmark-2026.md)) | **Wired, live** (Sept 2) — the starter term prices every remaining game with both probables posted. The four terms above it each clear the gate on the common game set and each stay out of production until a second season sizes them: all are < .0006 and inside 1 SE there. **Availability itself is worth nothing** (a pitch-count weight fires on 1,485 of 1,512 club-games where the old binary rest rule fired on 12, and scores the same); what pays is pricing the relief innings off component rates against the league. The start-length term is the only one whose *sign* is significant anywhere (t = −2.9 on 2025). The market still holds .0023 | `strength.home_win_prob`, `src/sim/starters.py`, `lineups.py`, `bullpen.py`, `reliever_usage.py`, `run_environment.py` |
